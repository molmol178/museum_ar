...
if __name__ == '__main__':
  input_img = cv2.imread('tools/risou_perspective.png',1)
  ...
  sum_one_dimention_scanning, sum_xy = featureDetection(input_gray)

  for i in xrange(len(sum_xy)):
    xy = sum_xy[i]
    change_xy = [xy[1],xy[0]]
    add_input = tuple(change_xy)
    cv2.circle(input_gray,add_input,3,(255,255,255),-1)
  cv2.imwrite('input_detection.tif', input_gray)

  keypoint_binary = featureDescription(sum_one_dimention_scanning, input_gray)
